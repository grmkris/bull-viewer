...
const BUDGETS: Budget[] = [
  ...
  {
    pattern: /^JsonViewer-.*\.js$/,
    label: "JsonViewer (CodeMirror)",
    maxKB: 200,
  },
  {
    pattern: /^FlowGraph-.*\.js$/,
    label: "FlowGraph (React Flow)",
    maxKB: 110,
  },
  {
    pattern: /^QueueOverview-.*\.js$/,
    label: "QueueOverview (uPlot)",
    maxKB: 60,
  },
];
...
for (const budget of BUDGETS) {
  ...
  if (!file) {
    console.error(
      `✗ ${budget.label}: no matching file in dist/lib (looking for ${budget.pattern})`
    );
    failed = true;
    continue;
  }
